  ...
  switch (type) {
    case UPDATE_EMAIL: {
      try {
        ...
        yield call(authService.updateEmail, payload.password, payload.newEmail);
    ...
    case UPDATE_PROFILE: {
      try {
        ...
        if (email && password && email !== state.profile.email) {
          yield call(authService.updateEmail, password, email);
        }

        if (avatarFile || bannerFile) {
          const bannerURL = bannerFile
            ? yield call(authService.uploadImage, bannerFile, 'banner')
            : payload.updates.banner;
          const avatarURL = avatarFile
            ? yield call(authService.uploadImage, avatarFile, 'avatar')
            : payload.updates.avatar;
          const updates = { ...payload.updates, avatar: avatarURL, banner: bannerURL };

          yield call(authService.updateProfile, state.auth.id, updates);
          yield put(updateProfileSuccess(updates));
        } else {
          yield call(authService.updateProfile, state.auth.id, payload.updates);
