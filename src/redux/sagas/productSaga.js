  ...
  switch (type) {
    case GET_PRODUCTS:
      try {
        ...
        const result = yield call(productService.getProducts, payload);
    ...
    case ADD_PRODUCT: {
      try {
        ...
        // The backend receives the raw image file(s) in one multipart
        // request and is responsible for storing them and returning URLs.
        const product = yield call(productService.addProduct, payload);
    ...
    case EDIT_PRODUCT: {
      try {
        ...
        const updatedProduct = yield call(productService.editProduct, payload.id, payload.updates);
    ...
    case REMOVE_PRODUCT: {
      try {
        yield initRequest();
        yield call(productService.removeProduct, payload);
    ...
    case SEARCH_PRODUCT: {
      try {
        ...
        const state = yield select();
        const result = yield call(productService.searchProducts, payload.searchKey);
